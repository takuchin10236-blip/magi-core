...
## v0.14.0 で追加：第3の色モード「残照」＋自動切替

社長採用 2026-08-08。仕様正本＝`開発/標準仕様/2026-08-08_候補_テーマ第3モード残照_仕様_v1.0.md`。
**White と Dark のあいだに夕焼けを足す**。命名は光の名で三対。

| 内部値（data-color-mode） | 表示名 | ロゴ | 中身 |
|---|---|---|---|
| `white`（既存・不変） | 陽光（ようこう） | day | 湘南グリーン基調 |
| `dusk`（v0.14.0 新設） | 残照（ざんしょう） | sunset | 暖クリーム地＋茜のヘッダー帯 |
| `dark`（既存・不変） | 月光（げっこう） | night | ネイビー基調 |

設計思想は「**舞台は夕焼け、道具は緑**」——背景・ヘッダーが暮れても、主操作ボタン・成功系・
リンクボタン・カード左帯は湘南グリーン `#2f6f5f` のまま（モードで操作の意味を動かさない＝01_UI標準 §2）。

```tsx
import { ColorModeSwitch, useThemeState } from '@magi/core/ui';

const theme = useThemeState();          // 既定は陽光。自動運転は職員が選んだときだけ動く
<ColorModeSwitch {...theme} />          // 陽光／残照／月光／自動 の4択
```

- **自動モード**: 陽光 6:00–16:00 → 残照 16:00–19:00 → 月光 19:00–6:00（端末時計・15分ごとに再評価）。
  時刻帯は `useThemeState({ autoSchedule: { dayStartHour, duskStartHour, nightStartHour } })` で調整できる。
- **手動が常に優先**。「自動」を選び直したときだけ帯運転へ戻る。
- `ColorModeSwitch` の「自動」ボタンは `onThemeModeSetting` を渡したときだけ出る。
  v0.13 以前のように `themeMode` / `onThemeMode` の2つだけで使っているアプリは**無改修**（3択になるだけ）。
- ヘッダーの帯は `--bg-header-sky`（茜の空）＋ `--bg-header-glow`（下端14px固定の夕陽の滲み）の2層。
  仕様§2 の1本ものの帯は `--bg-header-gradient` に逐語で残してある（固定高の帯を自前で描くとき用）。
  高さに依らない塗り方にした理由と実測値は CHANGELOG v0.14.0 を参照。
