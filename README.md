...
- `page`（`pageSize` / `nextPageToken`）と `searchScopeApplied` は未対応です。
- 現在のコンポーネントは、親から渡された1ページ分の候補だけをローカルで絞り込みます。
- サーバーページングを開始する場合は、Phase 2のB2接続時に、次ページ取得と検索範囲の適用結果を
  扱えるようコンポーネントを拡張する必要があります。
