...
import { AUTO_THEME_REEVALUATE_MS, DEFAULT_AUTO_THEME_SCHEDULE, DEFAULT_THEME_MODE_SETTING, DEFAULT_UI_PRESET, getUiPreset, normalizeThemeModeSetting, normalizeUiPreset, resolveAutoThemeMode, } from './uiPresets';
...
function themeModeSettingKey(prefix) {
...
function loadThemeModeSetting(prefix, fallback) {
    try {
...
export function useThemeState(options = {}) {
    const { storagePrefix = 'magi', defaultUiPreset = DEFAULT_UI_PRESET, defaultThemeMode = DEFAULT_THEME_MODE_SETTING, autoSchedule = DEFAULT_AUTO_THEME_SCHEDULE, autoIntervalMs = AUTO_THEME_REEVALUATE_MS, } = options;
    const [uiPreset, setUiPreset] = useState(() => loadUiPreset(storagePrefix, defaultUiPreset));
    const [themeModeSetting, setThemeModeSetting] = useState(() => loadThemeModeSetting(storagePrefix, defaultThemeMode));
    // 自動運転中の「今の帯」。初期値も時計から取る（初回描画から正しい色で出す）。
    const [autoMode, setAutoMode] = useState(() => resolveAutoThemeMode(new Date(), autoSchedule));
    // オブジェクトリテラルで渡されると毎描画で参照が変わるので、値で固定して effect の再実行を防ぐ。
    const { dayStartHour, duskStartHour, nightStartHour } = autoSchedule;
    const schedule = useMemo(() => ({ dayStartHour, duskStartHour, nightStartHour }), [dayStartHour, duskStartHour, nightStartHour]);
    const themeMode = themeModeSetting === 'auto' ? autoMode : themeModeSetting;
    ...
    const onThemeMode = useCallback((value) => {
        setThemeModeSetting(value);
    }, []);
    const onThemeModeSetting = useCallback((value) => {
        setThemeModeSetting(value);
    }, []);
    return {
        uiPreset,
