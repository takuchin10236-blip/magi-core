...
export function OperatorSelectModal({ open, onClose, onSelect, staff, selectedOperatorId, emptyMessage = '在籍職員の名簿を確認できません。更新してから管理者へ確認してください。', disclaimerCollapsible = false, }) {
    if (!open)
        return null;
    return (_jsx(DraggableModal, { maxWidth: "md", onClose: onClose, subtitle: "\u4FDD\u5B58\u306E\u8A18\u9332\u306B\u6B8B\u308A\u307E\u3059", title: "\u64CD\u4F5C\u8005\u3092\u9078\u629E", children: _jsxs("div", { className: "operator-select-body", children: [disclaimerCollapsible ? (_jsxs("details", { className: "operator-select-note operator-select-disclaimer is-collapsible", children: [_jsx("summary", { className: "operator-select-disclaimer-summary", children: DISCLAIMER_SUMMARY }), _jsx("p", { className: "operator-select-disclaimer-detail", children: DISCLAIMER_DETAIL })] })) : (_jsx("p", { className: "operator-select-note operator-select-disclaimer", role: "note", children: `${DISCLAIMER_SUMMARY}。${DISCLAIMER_DETAIL}` })), staff.length === 0 ? (_jsx("p", { className: "operator-select-note", children: emptyMessage })) : (_jsx("div", { "aria-label": "\u5728\u7C4D\u8077\u54E1\u306E\u4E00\u89A7", className: "operator-select-grid", role: "listbox", children: staff.map((member) => (_jsx("button", { "aria-selected": member.id === selectedOperatorId, className: `operator-select-item ${member.id === selectedOperatorId ? 'is-current' : ''}`, onClick: () => {
                            onSelect(member.id);
                            onClose();
