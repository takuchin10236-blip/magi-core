...
function noteDetectorRun(churn) {
    if (!isDevBuild() || churn.warned)
        return;
    const now = Date.now();
    ...
    churn.count += 1;
    if (churn.count <= DETECTOR_CHURN_LIMIT)
        return;
    churn.warned = true;
    console.warn('[MagiStatusSummary] writeDetector の参照が毎レンダー変わっています。'
