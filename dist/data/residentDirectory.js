...
export const RESIDENT_ACTIVE_STATUSES = ['入所中', '転入'];
...
function headerMissing() {
    throw apiError('RESIDENT_DIRECTORY_HEADER_MISSING', '利用者名簿の列を確認できないため、表示を停止しました。', 503);
}
function contractInvalid() {
    throw apiError('RESIDENT_DIRECTORY_CONTRACT_INVALID', '利用者名簿のIDを確認できないため、表示を停止しました。', 503);
}
function resolveHeaders(header) {
    const normalized = header.map((value) => cell(value).toLowerCase());
    const result = {};
    for (const key of Object.keys(RESIDENT_HEADER_ALIASES)) {
 ...
 */
export function parseResidentDirectoryRows(rows) {
    ...
    for (const raw of body) {
        ...
        if (!residentId && !name)
            continue;
        if (!residentId || residentId.length > MAX_ID_LENGTH || seen.has(residentId))
            contractInvalid();
        seen.add(residentId);
        if (!RESIDENT_ACTIVE_STATUSES.includes(status))
            continue;
...
//# sourceMappingURL=residentDirectory.js.map