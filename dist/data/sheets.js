 ...
 */
export function alignBatchGet(ranges, valueRanges) {
    if (valueRanges.length !== ranges.length) {
        throw apiError('SHEETS_API_ERROR', `batchGet length mismatch: requested ${ranges.length} ranges but got ${valueRanges.length}.`, 502);
    }
    ...
    for (const vr of valueRanges) {
        ...
        if (queue)
            queue.push(vr.values ?? []);
        else
            bySheet.set(sheet, [vr.values ?? []]);
    }
