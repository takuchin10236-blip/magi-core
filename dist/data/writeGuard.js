...
export function writeState(env, user, requireAdmin = false) {
    ...
    if (!user.allowed)
        blocks.push('USER_NOT_ALLOWED');
    if (requireAdmin && !user.admin)
        blocks.push('ADMIN_REQUIRED');
    if (!isTrue(env.ALLOW_WRITES))
        blocks.push('ALLOW_WRITES_FALSE');
    ...
    if (!spreadsheetId(env))
        blocks.push('SPREADSHEET_ID_MISSING');
    if (!googleConfigPresent(env))
        blocks.push('GOOGLE_SERVICE_ACCOUNT_MISSING');
    return {
        allowed: blocks.length === 0,
        blocks,
    };
}
export function assertAllowedUser(user) {
    if (!user.allowed) {
        throw apiError('AUTH_ERROR', 'このアプリを使う権限がありません。', 403);
    }
}
export function assertWriteAllowed(env, user, requireAdmin = false) {
    const state = writeState(env, user, requireAdmin);
    if (state.allowed)
        return;
    // ユーザ起因（権限）なら 403 AUTH_ERROR、それ以外（設定・環境）なら 423 WRITE_BLOCKED。
    const code = state.blocks.includes('USER_NOT_ALLOWED') || state.blocks.includes('ADMIN_REQUIRED')
        ? 'AUTH_ERROR'
        : 'WRITE_BLOCKED';
    throw apiError(code, `書込は許可されていません: ${state.blocks.join(', ')}`, code === 'AUTH_ERROR' ? 403 : 423);
}
