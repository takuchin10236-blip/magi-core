...
**保守枝（v0.25.x）へのバックポート**——`BusinessNav` が全画面ボタンの置き場所を持つ（main の v0.28.0 と同一の変更だけ）。

- `focusMode?` / `onFocusModeChange?` を追加。`onFocusModeChange` を渡した時だけ、右群の**先頭**に
  `FocusToggle` を core が描く（並び＝**全画面 → navActions → ロール → メニュー**・01_UI標準 §3-3-2）。
- `navLeadingActions` の JSDoc に「**全画面をここに置かない**」を明記。
- 渡さなければ描かない＝この枝を使う既存アプリは無風。
- **v0.24.0 の既定反転（`MagiStatusSummary` / `MagiBusinessSummary`）は入れていない**。

契機は 2026-09-06 社長裁定「全画面の位置、操作者の…形が違います。連絡ノートを正としてほしい。
どのアプリも、最初は、この形を取れるように」。採用: マニュアルアプリ。

## v0.25.2 (2026-09-05) — v0.25.0 系の保守枝

**v0.27.1 の移植1件のみ**（`patch/v0.25.x` 枝）。
...
## v0.25.1 (2026-09-05) — v0.25.0 系の保守枝

**v0.25.0 にピンを置いたまま、操作者チップの2件だけを届ける保守版**（`patch/v0.25.x` 枝）。
中身は v0.25.0 ＋ 下記2件（v0.26.0 / v0.27.0 の移植）で、main の同時点と同じ。
main が先へ進んでも採用アプリが巻き込まれないよう、**版を明示した札**として切ってある。
