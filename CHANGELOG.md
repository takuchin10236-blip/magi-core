...
- **`push` は「同じ断片なら書かない」ため、断片が同じで `state` だけ違う `navigate` は無視される**
  （マニュアルハブの既存挙動どおり）。付帯情報だけを差し替えたい時は `{ replace: true }` を使う
  （`replace` は常に書く）。
- **止めた時に履歴へ重複が1つ残る**（戻るで動いてしまった URL を積み直すため）。もう一度戻れば
  もう一度尋ねる＝安全側に倒れる形で、実ブラウザの `beforeunload` と同じ割り切り。
