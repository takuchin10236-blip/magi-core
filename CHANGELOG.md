...
**検査が「型どおりの新しい形」を罰していたのを是正**（v0.28.0 の穴・CIガード (j) の1行）。**API 変更なし**。

### 契機

v0.28.0 で全画面ボタンの置き場所を型へ移した——採用アプリは `FocusToggle` を自分で置くのをやめ、
`onFocusModeChange` を `BusinessNav` へ渡すだけになる。ところが CIガード (j)（ナビ・メニュー標準）は
**`FocusToggle` という綴りが src に在るか**だけを見ていた。

結果、**型に追従したアプリほど「全画面（ナビ直置き）が無い」と警告される**。
利用者マスタ・職員マスタ・フロアカレンダーを v0.28.0 へ追従させた時に3本とも同じ WARN が出て掴んだ。
警告に従って `TYPE_DEVIATIONS.md` へ「持たない」と書けば、**型どおりなのに逸脱として記録が残る**——
検査が嘘を書かせる状態だった。

### 変更

- `ci/check-ui-guardrails.mjs` の (j) で、標準装備の1行を「**充足する綴りのどれか1つ**」で持つ形へ。
  全画面は **`onFocusModeChange` または `FocusToggle`** のどちらかがあれば充足。
  欠けている時のメッセージは両方の綴りを案内する。
- 他の5部品（ManualEntry / ColorModeSwitch / VersionHistoryModal / SgBrandLogo / MagiBusinessSummary）は
  判定も文言も従来どおり（1綴りの配列になっただけ）。

### 検査

`test/navMenuStandardGuard.test.ts` に2件追加（型どおりの新しい形で充足すること・欠け時に両綴りを案内すること）。
**変異試験で検出力を確認済み**——判定を旧綴りだけへ戻すと新規2件が赤になる。
...
v0.27.0 の縮小算法を実ブラウザで測っていて、**ケアプロファイルの画面だけ 13字が枠から出る**のを掴んだ。
原因は算法ではなく CSS——`@media (prefers-reduced-motion: reduce)` の
`* { transition-duration: 1ms !important }` が、**全プロパティに遷移を掛ける**
（`transition-property` の初期値は `all`）。`font-size` にも掛かるので、`operatorFit` が
字を小さくしても**同じタスクの中では遷移前の値のまま測れてしまい**、縮小ループが1段で止まる。

つまり **端末で「動きを減らす」を入れている職員だけ、長い名前が切れて出る**。
写真にも机上の試験にも出ない——実ブラウザで数えて初めて見えた。

### 変更

- `.operator-chip.is-fixed .operator-chip-label` に **`transition-property: none !important`**。
  字の大きさは「動かす」ものではないので、遷移の対象から外す。

### 検査

実ブラウザ（`prefers-reduced-motion: reduce` 有効）で 6/13/17/20字を測り、**すべて枠に収まる**ことを確認。
採用アプリ側の `qa:visual` は動きを減らす設定で走っているので、そこが常時の見張りになる。

## v0.27.0 (2026-09-05)

**操作者チップの名前を「枠に必ず収める」**（2026-09-05 社長裁定「操作者は、大きさはそのまま、文字数が多い職員は、文字をその枠に合わせて調整してほしい」）。**API と輸出の破壊はなし**（追加輸出のみ）。

### 契機

幅180pxのチップで、**17字の職員名が枠外へ出て切れていた**。縮小ループの下限が 10px で、
そこで止まったあと**収まりきらないまま抜けていた**ため。`text-overflow: ellipsis` も無いので
「…」すら出ず、**切れたことに誰も気づけない**。施設には外国籍職員の長いカタカナ氏名が実在する。

### 変更

- **`src/ui/operatorFit.ts` を新設**——収める算法を DOM から切り離した（jsdom はレイアウトを
  計算しないので `scrollWidth` が常に 0＝机上の試験では絶対に見つからない事故だった）。
  - `FIT_MIN_PX` を **10px → 7px**（読めなくなる下限）。
  - 下限まで縮めても入らない長さは、**字間を詰めて枠へ入れ切る**（はみ出し量÷文字数で当たりを付け、
    0.1px 刻み・上限24回で詰め切る）。**「収まりきらないまま抜ける」経路を無くした**。
- `src/ui/Operator.tsx`: 縮小ループを `fitOperatorLabel(domFitTarget(span))` の1行へ置換。
- `src/ui/index.ts`: `fitOperatorLabel` / `domFitTarget` / `FIT_MIN_PX` / `FIT_BASE_PX` と型2本を輸出。
- **`scripts/verify-operator-fit.mjs` を新設**（`npm run verify:operator-fit`）——実ブラウザで
  `scrollWidth <= clientWidth` を測る。CSS も定数も**現物を読む**（写しを置くと直したのに試験は昔のまま）。
  playwright-core は core の依存に入れず、採用アプリの node_modules を借りる（`dev/screenshot.mjs` と同じ事情）。
  借りられない時は **exit 2**（「測れなかった」を緑にしない）。`check` には入れていない（兄弟repo依存のため）。
- `test/operatorFit.test.ts` を新設（15試験）。**負例1本を含む**——旧実装（下限10pxで抜ける）を
  模して 17字が枠を超えることを確かめる＝この試験が効いていることの証拠。

### 実測（実ブラウザ・チップ幅180px／ラベル内寸132px／全角かな1文字＝1em の最悪ケース）
...
**字の大きさだけで収まるのは18字まで。19字以上は字間を詰めて入れ切る**（20字で -0.4px＝-0.057em＝目には見えない差）。

### 採用側への影響

- 見た目が変わるのは**14字以上の名前だけ**（それ以下は従来と同じ大きさ）。API の変更は無い。
...
連絡ノートで固まった形——節の題名は素の名詞（「出勤したら」「基本の投稿」）、末尾は共通節3本固定——を
各アプリが手写しで守る状態だった。手写しは必ず枝分かれする（丸数字が戻る・共通節の並びが入れ替わる・
共通節の写しをアプリが持つ）。**様式違反をビルドで落とす**形にして型へ上げる。

### 変更

- **`composeAppManual(meta, appSections, commonTail)`**（新）: アプリ内マニュアルを標準様式で組み立てる。
  `{ ...meta, sections: [...appSections, ...commonTail] }` を返す前に4点を検証し、違反は**日本語メッセージで throw**。
  各アプリはモジュール初期化時に呼ぶ（`export const MANUAL = composeAppManual(...)`）＝**ビルドと試験で必ず落ちる物理ガード**
  （画面を開いた人しか気づけない `console.warn` にはしない）。
  - (a) アプリ固有節の `title` / `summary` に**丸数字を含まない**（①〜⑳・⓪・㉑〜㉟・㊱〜㊿。検出した文字と節idをメッセージに出す）。番号は器が自動で振る。
  - (b) `commonTail` の id が **`floor-account` → `entry-key` → `logo-column` の順で完全一致**（並べ替え・欠け・別物は throw）。
  - (c) アプリ固有節に上の**予約id が混ざっていない**（自前の写しで共通節を上書きさせない）。
  - (d) 全節の **id が重複しない**（重複すると目次ジャンプの宛先が壊れる）。
- **`MANUAL_COMMON_TAIL_IDS`**（新・`readonly ['floor-account','entry-key','logo-column']`）: 末尾共通節の契約。
- **core は `@magi/manual-content` に依存しない**——共通節の**実体（文面・図）は同パッケージが配り**、
  core が持つのは id の並びという契約だけ（公開リポジトリに施設固有の文面を置かないため）。
  並びの正本＝**02番マニュアル標準 §3-4(5-3) v1.6**（アプリ固有 → 共通アカウント → 入口のカギ → ロゴ最末尾）。

### 採用側への影響

**新API追加のみ・既存への影響なし**（`ManualContent` を手組みしている既存アプリはそのまま動く）。
今後は**アプリ内マニュアルは `composeAppManual` 経由を標準とする**——版を上げた卓から順に、
`{ appName, appVersion, subtitle, sections }` の手組みをこの関数の呼び出しへ置き換える。
置き換えた時点で丸数字と共通節の並びが機械で守られる（違反は import 時に throw＝ビルドが赤くなる）。

### 検査

`npm run build` OK／`verify:brand` OK／`verify:modal` 20 checks／`verify:shell` 25 checks／
`verify:tokens` 30 checks／`verify:types` OK／**381試験（新規7本＝正例1・負例6）**。

⚠️ **`verify:matrix` はこの機（Mac mini）では通らない**——`source_hashes` が現在の読取元と不一致。
v0.21.0 以降と同じ既知の状態で、**本版の変更が原因ではない**。版SoTの再生成は**行っていない**。

## v0.24.0 (2026-09-01)

**v0.23.0 の opt-in 3点を「既定ON」へ昇格**（2026-09-01 社長裁定「**どのアプリでも、反映されるように正式なMAGIの型にして**」）。opt-out（`={false}`）は3点とも残す。**既定値の反転だけ**——判定ロジックと fail-closed は変えていない。ただし既定が変わることで、**これまで既定OFFのせいで眠っていた経路（`hidden` の controlled）が有効になる**点は下の影響節を読むこと。

### 契機

v0.23.0 で compact / collapsible / fixedWidth を opt-in で入れたところ、採用したのは連絡ノートだけだった。
他アプリはライブラリを上げても旧見た目のまま＝「型に昇格したのに反映されない」。
アプリ側が何も渡さなくても新様式になる形へ既定を反転する。**各アプリは core の版をピンで固定している**ため、
既定反転が勝手に他アプリの本番へ流れ込むことはない（見た目が変わるのは各卓が版を上げると決めた時点だけ）。

### 変更

- `MagiStatusSummary` の **`compact` が既定 `true`**（`compact={false}` で従来の個別バッジへ戻せる）。
  **fail-closed は不変**——環境と書込が両方とも機械検出で確定した時だけ1枚化し、確認中・検出失敗・
  無検証申告・申告エラーが1つでもあれば自動で個別展開する。この安全弁が内側にあるから既定にできる。
- `OperatorChip` の **`fixedWidth` が既定 `true`**、型を `number | true` → **`number | boolean`** へ拡張。
  `fixedWidth={false}` で従来表示（「操作者: 名前」／未選択は `unsetLabel`）へ戻せる。数値指定の幅固定は従来どおり。
  内部判定を `fixedWidth !== undefined` → `fixedWidth !== false` へ、幅の明示を `typeof fixedWidth === 'number'` の時だけへ
  （既定 `true` のまま `width: undefinedpx` を書かないための整理。**`style.width` が空であることを見る回帰ピンつき**——
  旧式の書き方へ戻す変異を検出する試験ではなく、幅の書き込み条件が将来ずれたら落ちるピン）。
  **`unsetLabel` は従来表示でのみ使う prop になった。**
- `MagiBusinessSummary` の **`collapsible` が既定 `true`**（`collapsible={false}` で従来どおり隠せない帯へ）。
  帯の初期状態は従来と同じ「表示」＝出た違いは「隠す」ボタンが増えることだけ。
  **`hidden` は v0.23.0 では `collapsible` 未指定時に効かなかった**（隠すゲートが `collapsible && effectiveHidden` のため）。
  既定ON化で**有効になる＝`hidden` だけ渡していたアプリは、版を上げると帯が消える**（`hidden` の描画そのもの＝
  隠し中は行ごと描かない・`onHiddenChange` の呼び方は不変）。意図せず消える場合は `hidden={false}` を渡すか
  `collapsible={false}` で opt-out する。
- `BusinessNavTab` の **`emphasis` は opt-in のまま**（どのタブが主操作かはアプリの意味づけで、型として既定化できない）。

### 採用側の検査への影響（議題F様式）

**版を上げた時点で全アプリに適用される**（v0.23.0 では「採用したアプリだけ」だった点が変わる）。
**現行の採用アプリの多くは v0.18.1 ピン**＝版を上げる時は本節だけでなく **v0.19.0 以降の各節の影響も併せて読むこと**
（ManualViewer の「閉じる×」→「← 戻る」、`.operator-chip.is-unset` の枠色 黄→赤、メニューパネル幅300px 等）。

- **操作者チップの表示文言**: 「操作者: 名前」→「**名前**」だけ／未選択「操作者: 未選択」→「**操作者**」。
  **`操作者:` の文字列を待っている画面検査・visual-qa は、版を上げた時点で要更新**。従来文言を続けるなら `fixedWidth={false}`。
- **操作者チップが幅230px固定になる**（`.operator-chip.is-fixed`・CSS は v0.23.0 で既に入っている）。
  ヘッダー右群の横幅・スクリーンショットを実測する検査は要更新。幅を変えたいアプリは `fixedWidth={280}` のように数値で渡す。
- **状態バッジが1枚に畳まれ得る**: 「本番URL」「書込ON」の個別文字列を待つ検査は、条件が揃うと
  「**本番・書込ON**」1枚になるため要更新（不確定時は従来どおり個別展開のまま）。従来表示を続けるなら `compact={false}`。
- **「現在の状況」帯に「隠す」ボタンが増える**。帯の要素数・スクリーンショットを固定している検査は要更新。
- **`hidden` だけ渡していたアプリは帯が消える**（上の「変更」節参照）。
- **opt-out は `={false}` の明示だけ**。`undefined` は既定ON に解決される
  ——`fixedWidth={flag ? 320 : undefined}` は「flag が偽なら従来表示」にはならず**新様式のまま**になる（罠）。
  条件で切り替えるなら `fixedWidth={flag ? 320 : false}` と書く。

### 検査

`npm run build` OK／`verify:brand` OK／`verify:modal` 20 checks／`verify:shell` 25 checks／
`verify:tokens` 30 checks／`verify:types` OK／**374試験（新規12本）**。
新試験は変異試験で検出力を確認済み——①既定を `false` へ戻すと6本が落ちる ②compact のラベル導出を
旧式（三項連鎖）へ戻して unknown ガードを消すと、unknown の負例1本が落ちる（この便より前は全緑だった穴）。
なお**新構造ではガード行だけ消しても表引きが undefined を返して畳まない**（試験は全緑のまま安全側）＋
`verify:types` が TS2339 で落ちる＝二重の網になっている。

⚠️ **`verify:matrix` はこの機（Mac mini）では通らない**——`source_hashes` が現在の読取元と不一致。
**本版の変更が原因ではなく、着手前の clean な状態（v0.23.0 の HEAD）で実測しても同じ**。
版SoTの再生成は**行っていない**（素で再生成すると検証記録が消えるため＝2026-08-20 に同じ事故）。
