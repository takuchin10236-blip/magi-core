...
**保守枝（v0.23.x）へのバックポート**——`BusinessNav` が全画面ボタンの置き場所を持つ（main の v0.28.0 と同一の変更だけ）。
...
- **v0.24.0 の既定反転（`MagiStatusSummary` / `MagiBusinessSummary`）は入れていない**——この枝は
  古い既定のまま留めるための枝なので、混ぜない。

契機は 2026-09-06 社長裁定「全画面の位置、操作者の…形が違います。連絡ノートを正としてほしい。
どのアプリも、最初は、この形を取れるように」。採用: 連絡ノート・ケアプロファイル。
