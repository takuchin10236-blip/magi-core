 ...
 */
export async function snapshotHash(values: SheetValues): Promise<string> {
...
export async function assertFreshSnapshot(
  source: Pick<MagiDataSource, 'read'>,
  range: string,
  expected: string,
): Promise<void> {
  ...
  throw apiError(
    'STALE_SNAPSHOT' satisfies ConcurrencyReason,
...
function normalizeValues(values: SheetValues): string[][] {
