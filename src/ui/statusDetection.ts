...
export function detectRuntime(
  ...
  if (config.productionHosts?.includes(host)) return 'production';
  if (config.previewHosts?.includes(host)) return 'preview';
  if (config.localHosts?.includes(host) || isLocalHost(host)) return 'local';
  return 'unknown';
}
