 ...
 */
export const SG_BRAND_ICON_SOURCES: Record<SgBrandIconVariant, SgBrandIconSource> = {
...
const VARIANT_BY_THEME: Record<ThemeMode, SgBrandIconVariant> = {
  white: 'day',
  dusk: 'sunset',
  dark: 'night',
};
...
const MANAGED_ATTRS = ['href', 'type', 'sizes'] as const;

let ownedLink: HTMLLinkElement | null = null;
let createdByUs = false;
let originalAttrs: Array<[string, string | null]> = [];
let holders = 0;

function acquireLink(): HTMLLinkElement | null {
  if (typeof document === 'undefined') return null;
  if (holders === 0) {
    const existing = document.head.querySelector<HTMLLinkElement>(ICON_LINK_SELECTOR);
    if (existing) {
...
function releaseLink(): void {
  if (holders === 0) return;
  holders -= 1;
  if (holders > 0 || !ownedLink) return;
  if (createdByUs) {
    ownedLink.remove();
  } else {
    // 借り物は借りた時の姿へ返す（href だけでなく type/sizes も。目印も消す）。
    for (const [name, value] of originalAttrs) {
      if (value === null) ownedLink.removeAttribute(name);
      else ownedLink.setAttribute(name, value);
    }
 ...
 */
export function useBrandFavicon(options: UseBrandFaviconOptions = {}): SgBrandIconVariant {
