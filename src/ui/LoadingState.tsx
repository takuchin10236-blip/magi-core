...
export function LoadingState({ label, slowHint, variant = 'block', className }: LoadingStateProps) {
  return (
    <div
      aria-live="polite"
      className={`magi-loading magi-loading-${variant}${className ? ` ${className}` : ''}`}
      role="status"
    >
      <span aria-hidden="true" className="magi-loading-spinner" />
      <span className="magi-loading-text">
        <span className="magi-loading-label">{label}</span>
        {slowHint ? <span className="magi-loading-hint">{slowHint}</span> : null}
      </span>
    </div>
  );
}
