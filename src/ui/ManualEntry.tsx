...
export function ManualEntry({ content, label = 'マニュアル', renderTrigger }: ManualEntryProps) {
  ...
  return (
    <>
      {renderTrigger ? (
        renderTrigger(openViewer)
      ) : (
        <button
          type="button"
          className="magi-manual-entry"
          onClick={openViewer}
          title={`${content.appName} のマニュアルを開きます`}
          aria-haspopup="dialog"
          aria-expanded={open}
        >
          {/* 本（マニュアル）アイコン。lucide 非依存のためインライン SVG で描く。 */}
          <svg
            className="magi-manual-entry__icon"
            width="20"
            height="20"
            viewBox="0 0 24 24"
            fill="none"
          ...
          <span className="magi-manual-entry__label">{label}</span>
        </button>
      )}
      {open &&
        typeof document !== 'undefined' &&
        createPortal(<ManualViewer content={content} onClose={closeViewer} />, document.body)}
    </>
  );
}
