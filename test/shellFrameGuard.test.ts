...
describe('(h) シェルの枠: 標準どおりの寸法指定は通す（基準実体の書き方）', () => {
  ...
  it('トークンで書けば警告も出ない', () => {
    const { code, out } = run('.magi-appshell { padding: var(--magi-shell-padding); }');
    expect(code).toBe(0);
  ...
  it('.magi-appshell-main の padding: 0 は枠を壊さない（Coreは padding を持たない）', () => {
    const { code } = run('.work-view .magi-appshell-main { padding: 0; }');
    expect(code).toBe(0);
  });
});
...
describe('(h) 例外は承認機構で', () => {
  it('TYPE_DEVIATIONS.md に UI-SHELL-FRAME の承認があれば警告で通す', () => {
    const deviations = [
      ...
      '| --- | --- | --- | --- | --- | --- |',
      '| UI-SHELL-FRAME | シェル枠の解除 | 全画面の帳票プレビュー | 区分C | 2026-07-30 | 承認済 |',
    ].join('\n');
