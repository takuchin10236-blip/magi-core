...
const FIXED_RED: Array<{ what: string; detail: string }> = [
  ...
  {
    what: 'warning の面を #d84315 → #be3b13 へ暗くし、トーストが 3.64:1 → 5.49:1（面を動かした唯一の項目）',
    detail:
      '旧・面 --warn(#d84315) は中輝度で、白 4.44:1 / 混色8% 4.52:1 / 純黒 4.73:1 ＝**どんな文字色でも上限 4.73:1**（実測）。文字側では余裕が 0.23 しか作れないため、次便で**面そのもの**を直した（社長承認・状態色の面を動かすのはこれだけ）。色相14.15°・彩度82.3% を固定し HSL の明度だけ 46.5%→41.0% へ下げた #be3b13＝白文字 5.49:1 / 黒 3.82:1 ＝**新しい上限は 5.49:1**（余裕 0.99）。面が暗くなったのでインクは純黒→白へ言い直した。明度は 0.5% 刻みで18段の候補を全計測して決めており、ぎりぎり通る 42.0%(#c33d13) は最悪組合せ（nova-ember 陽光のカード地）が 4.52:1＝余裕 0.02 で「別の崖」になるため採らなかった（採用値の最悪は 4.73:1）。例外は nova-carbon 陽光だけで、この preset は warning の面が --accent(#ef4444) のため純黒のまま。',
  },
  {
